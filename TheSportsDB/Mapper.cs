...
namespace TheSportsDB
{
    internal static class Mapper
    {
        ...
        public static Honor MapHonor(JObject item)
        {
            var newHonor = new Honor();

            if (item["id"].ToObject<object>() != null)
                newHonor.Id = Convert.ToInt32(item["id"]);

            if (item["idPlayer"].ToObject<object>() != null)
                newHonor.IdPlayer = Convert.ToInt32(item["idPlayer"]);

            if (item["idTeam"].ToObject<object>() != null)
                newHonor.IdTeam = Convert.ToInt32(item["idTeam"]);

            if (item["strSport"].ToObject<object>() != null)
                newHonor.Sport = item["strSport"].ToString();

            if (item["strPlayer"].ToObject<object>() != null)
                newHonor.Player = item["strPlayer"].ToString();

            if (item["strTeam"].ToObject<object>() != null)
                newHonor.Team = item["strTeam"].ToString();

            if (item["strHonour"].ToObject<object>() != null)
                newHonor.Honour = item["strHonour"].ToString();

            if (item["strSeason"].ToObject<object>() != null)
                newHonor.Season = item["strSeason"].ToString();

            return newHonor;
        }

        public static FormerTeam MapFormerTeam(JObject item)
        {
            var newFormerTeam = new FormerTeam();

            if (item["id"].ToObject<object>() != null)
                newFormerTeam.Id = Convert.ToInt32(item["id"]);

            if (item["idPlayer"].ToObject<object>() != null)
                newFormerTeam.IdPlayer = Convert.ToInt32(item["idPlayer"]);

            if (item["idFormerTeam"].ToObject<object>() != null)
                newFormerTeam.IdFormerTeam = Convert.ToInt32(item["idFormerTeam"]);

            if (item["strSport"].ToObject<object>() != null)
                newFormerTeam.Sport = item["strSport"].ToString();

            if (item["strPlayer"].ToObject<object>() != null)
                newFormerTeam.Player = item["strPlayer"].ToString();

            if (item["strFormerTeam"].ToObject<object>() != null)
                newFormerTeam.FormerTeamName = item["strFormerTeam"].ToString();

            if (item["strMoveType"].ToObject<object>() != null)
                newFormerTeam.MoveType = item["strMoveType"].ToString();

            if (item["strTeamBadge"].ToObject<object>() != null)
                newFormerTeam.TeamBadge = item["strTeamBadge"].ToString();

            if (item["strJoined"].ToObject<object>() != null)
                newFormerTeam.Joined = item["strJoined"].ToString();

            if (item["strDeparted"].ToObject<object>() != null)
                newFormerTeam.Departed = item["strDeparted"].ToString();

            return newFormerTeam;
        }

        public static Country MapCountry(JObject item)
        {
            var newCountry = new Country();

            if (item["name_en"].ToObject<object>() != null)
                newCountry.Name = item["name_en"].ToString();

            return newCountry;
        }

        public static Contract MapContract(JObject item)
        {
            var newContract = new Contract();

            if (item["id"].ToObject<object>() != null)
                newContract.Id = Convert.ToInt32(item["id"]);

            if (item["idPlayer"].ToObject<object>() != null)
                newContract.IdPlayer = Convert.ToInt32(item["idPlayer"]);

            if (item["strSport"].ToObject<object>() != null)
                newContract.Sport = item["strSport"].ToString();

            if (item["strPlayer"].ToObject<object>() != null)
                newContract.Player = item["strPlayer"].ToString();

            if (item["strTeam"].ToObject<object>() != null)
                newContract.Team = item["strTeam"].ToString();

            if (item["strTeamBadge"].ToObject<object>() != null)
                newContract.TeamBadge = item["strTeamBadge"].ToString();

            if (item["strYearStart"].ToObject<object>() != null)
                newContract.YearStart = item["strYearStart"].ToString();

            if (item["strYearEnd"].ToObject<object>() != null)
                newContract.YearEnd = item["strYearEnd"].ToString();

            if (item["strWage"].ToObject<object>() != null)
                newContract.Wage = item["strWage"].ToString();
